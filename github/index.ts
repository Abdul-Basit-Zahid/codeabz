...
import { createcodeabzClient } from "@codeabz/sdk"
...
const { client, server } = createcodeabz()
...
function createcodeabz() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`codeabz`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createcodeabzClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/codeabz|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/codeabz` or `/oc`")
  }
}
...
async function assertcodeabzConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to codeabz server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.github.com/Abdul-Basit-Zahid/codeabz" : "https://github.com/Abdul-Basit-Zahid/codeabz"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.github.com/Abdul-Basit-Zahid/codeabz/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("codeabz-github-action")
    response = await fetch("https://api.github.com/Abdul-Basit-Zahid/codeabz/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/codeabz" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/codeabz") || body.includes("/oc")) {
      if (reviewContext) {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `codeabz/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/codeabz-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[codeabz session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
