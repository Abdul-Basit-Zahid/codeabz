...
const model = "codeabz/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/codeabz")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with codeabz...`)
  ...
  try {
    await $`codeabz run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  codeabz failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with codeabz")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with codeabz...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/codeabz`.",
