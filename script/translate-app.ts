...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "codeabz/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".codeabz/glossary/zh-cn.md"
  if (locale === "zht") return ".codeabz/glossary/zh-tw.md"
  return `.codeabz/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("codeabz did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("codeabz returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("codeabz session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel codeabz runs for 'all' (default: 4)
      --model <provider/id>  codeabz model (default: codeabz/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running codeabz
...
`)
  ...
  if (failed.length) console.error(`\ncodeabz failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.CODEABZ_DISABLE_PROJECT_CONFIG = "1"
  env.CODEABZ_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["codeabz", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.CODEABZ_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["codeabz", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.CODEABZ_CONFIG
  delete env.CODEABZ_CONFIG_DIR
  delete env.CODEABZ_CONFIG_CONTENT
  delete env.CODEABZ_PERMISSION
  delete env.CODEABZ_AUTO_SHARE
  return env
}
