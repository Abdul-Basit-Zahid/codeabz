...
# パッケージマネージャー
npm i -g codeabz@latest        # bun/pnpm/yarn でもOK
scoop install codeabz             # Windows
choco install codeabz             # Windows
brew install anomalyco/tap/codeabz # macOS と Linux（推奨。常に最新）
brew install codeabz              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S codeabz            # Arch Linux (Stable)
paru -S codeabz-bin               # Arch Linux (Latest from AUR)
mise use -g codeabz               # どのOSでも
nix run nixpkgs#codeabz           # または github:Abdul-Basit-Zahid/codeabz で最新 dev ブランチ
```
...
codeabz はデスクトップアプリとしても利用できます。[releases page](https://github.com/Abdul-Basit-Zahid/codeabz/releases) から直接ダウンロードするか、[github.com/Abdul-Basit-Zahid/codeabz/download](https://github.com/Abdul-Basit-Zahid/codeabz/download) を利用してください。
...
1. `$codeabz_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.codeabz/bin` - デフォルトのフォールバック
...
codeabz には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://github.com/Abdul-Basit-Zahid/codeabz/docs/agents) の詳細はこちら。

### ドキュメント

codeabz の設定については [**ドキュメント**](https://github.com/Abdul-Basit-Zahid/codeabz/docs) を参照してください。

### コントリビュート

codeabz に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### codeabz の上に構築する

codeabz に関連するプロジェクトで、名前に "codeabz"（例: "codeabz-dashboard" や "codeabz-mobile"）を含める場合は、そのプロジェクトが codeabz チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/codeabz) | [X.com](https://x.com/codeabz)
