...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codeabz-find")
  api.delete("codeabz-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codeabz-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codeabz-find", new Highlight(...rest))
    return true
  }
