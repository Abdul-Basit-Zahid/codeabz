...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@codeabz/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/codeabz`)).toBe("path")
    expect(inlineCodeKind(`@codeabz/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://github.com/Abdul-Basit-Zahid/codeabz/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/codeabz`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://github.com/Abdul-Basit-Zahid/codeabz/docs`)).toBeUndefined()
  })
})
