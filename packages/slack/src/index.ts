...
import { createcodeabz, type ToolPart } from "@codeabz/sdk"
...
console.log("🚀 Starting codeabz server...")
const codeabz = await createcodeabz({
  port: 0,
})
console.log("✅ codeabz server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await codeabz.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new codeabz session...")
    const { client, server } = codeabz
    ...
    console.log("✅ Created codeabz session:", createResult.data.id)
  ...
  console.log("📝 Sending to codeabz:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 codeabz response:", JSON.stringify(result, null, 2))
