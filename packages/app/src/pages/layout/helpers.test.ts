...
import { type Session } from "@codeabz/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codeabz://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codeabz://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codeabz://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codeabz://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codeabz://open-project")).toBeUndefined()
    expect(parseDeepLink("codeabz://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codeabz://open-project?directory=/a",
      "codeabz://other?directory=/b",
      "codeabz://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codeabz://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codeabz://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codeabz://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codeabz://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codeabz://new-session?directory=/a",
      "codeabz://open-project?directory=/b",
      "codeabz://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODEABZ__: {
        deepLinks: ["codeabz://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODEABZ__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codeabz://open-project?directory=/a"])
