...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-codeabz" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslcodeabzAction(check?: WslcodeabzCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install codeabz"
  if (check.matchesDesktop === false) return "Update codeabz"
}
...
export function addServerViewModel(input: {
  ...
  const codeabzCheck = selectedDistro ? (state?.codeabzChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.codeabzChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updatecodeabz" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.codeabzMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installcodeabz" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  codeabzCheck: WslcodeabzCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingcodeabz =
    input.state?.job?.kind === "install-codeabz" && input.state.job.distro === input.selectedDistro
  if (!ready || probingcodeabz) {
    return {
      variant: "contrast",
  ...
  if (!addServercodeabzReady(input.codeabzCheck)) {
    const update = !!input.codeabzCheck?.resolvedPath && input.codeabzCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingcodeabz
        ? { key: "wsl.onboarding.updatingcodeabz" }
        : update
          ? { key: "wsl.onboarding.updatecodeabz" }
          : { key: "wsl.onboarding.installcodeabz" },
      disabled: !!input.state?.job || input.adding,
      action: "install-codeabz",
      loading: installingcodeabz,
...
function addServercodeabzReady(check: WslcodeabzCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.codeabzChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.codeabzChecks[item.name]) return [`codeabz:${item.name}`]
    return []
  })
