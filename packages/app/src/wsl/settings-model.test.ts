...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when codeabz needs attention", () => {
    expect(wslcodeabzAction(undefined)).toBeUndefined()
    expect(
      wslcodeabzAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install codeabz")
    expect(
      wslcodeabzAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/codeabz",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when codeabz is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        codeabzChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.codeabz/bin/codeabz",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installcodeabz" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-codeabz")
  })
