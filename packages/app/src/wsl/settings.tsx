...
import { Tag } from "@codeabz/ui/v2/badge-v2"
...
import { wslcodeabzAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.codeabzChecks[item.config.distro]
          const codeabzAction = () => wslcodeabzAction(check())
          const busy = () => wsl.data?.job?.kind === "install-codeabz" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={codeabzAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installcodeabz(item.config.distro))}
                    >
