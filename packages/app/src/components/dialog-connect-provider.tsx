import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@codeabz/sdk/v2/client"
...
import { List, type ListRef } from "@codeabz/ui/list"
import { ProviderIcon } from "@codeabz/ui/provider-icon"
import { Spinner } from "@codeabz/ui/spinner"
import { Tag } from "@codeabz/ui/tag"
...
function ProviderPicker(props: { directory?: Accessor<string | undefined>; onSelect: (provider: string) => void }) {
  ...
  const note = (id: string) => {
    ...
    if (id === "codeabz-go") return language.t("dialog.provider.codeabzGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "codeabz"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.codeabz.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "codeabz"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codeabzZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codeabzZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.codeabzZen.visit.prefix")}
                <Link href="https://github.com/Abdul-Basit-Zahid/codeabz/zen" tabIndex={-1}>
                  {language.t("provider.connect.codeabzZen.visit.link")}
                </Link>
                {language.t("provider.connect.codeabzZen.visit.suffix")}
              </div>
            </div>
          </Match>
          <Match when={true}>
