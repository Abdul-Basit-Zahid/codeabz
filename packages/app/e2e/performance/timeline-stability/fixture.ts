...
import { SessionStatusEvent } from "@codeabz/schema/session-status-event"
import { SessionV1 } from "@codeabz/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockcodeabzServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/codeabz/TimelineStability"
...
export const model = { providerID: "codeabz", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("codeabz.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["codeabz"],
    default: { providerID: "codeabz", modelID: "claude-opus-4-6" },
  }
}
