...
const directory = "C:/codeabz/ReviewOpenFile"
...
test("opens and searches project files inline", async ({ page }) => {
  ...
  await mockcodeabzServer(page, {
    ...
    provider: {
      ...
      connected: ["codeabz"],
      default: { providerID: "codeabz", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "codeabz.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "codeabz.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: false, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "codeabz.window.browser.dat:tabs",
