...
import type { IntegrationOAuthMethodRegistration } from "@codeabz/plugin/v2/effect/integration"
import { define } from "@codeabz/plugin/v2/effect/plugin"
import type { CredentialValue } from "@codeabz/sdk/v2/types"
...
const defaultServer = "https://console.github.com/Abdul-Basit-Zahid/codeabz"
const clientID = "codeabz-cli"
...
export const codeabzPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "codeabz",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("codeabzPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("codeabz")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load codeabz provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("codeabz", (integration) => {
        integration.name = "codeabz"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "codeabz", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("codeabz")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("codeabz")
      ...
      const item = catalog.provider.get(ProviderV2.ID.codeabz)
      if (!item) return
      const hasKey = Boolean(process.env.CODEABZ_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("codeabz")),
