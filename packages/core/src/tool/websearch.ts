...
import { ToolFailure } from "@codeabz/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@codeabz/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CODEABZ_WEBSEARCH_PROVIDER === "exa" || process.env.CODEABZ_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CODEABZ_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CODEABZ_EXPERIMENTAL") || truthy("CODEABZ_ENABLE_EXA") || truthy("CODEABZ_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CODEABZ_ENABLE_PARALLEL") || truthy("CODEABZ_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `codeabz/${InstallationVersion}`,
