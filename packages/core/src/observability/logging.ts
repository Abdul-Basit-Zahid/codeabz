...
export function fileLogger(file = path.join(Global.Path.log, "codeabz.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.CODEABZ_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.CODEABZ_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
