...
import { Watcher } from "@codeabz/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    CODEABZ_EXPERIMENTAL_FILEWATCHER: "true",
    CODEABZ_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@codeabz.test`.cwd(tmp.path).quiet()
