...
import { Policy } from "@codeabz/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads codeabz JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "codeabz.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "codeabz.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "codeabz.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "codeabz.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "codeabz.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "codeabz.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .codeabz configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".codeabz"), { recursive: true })
            await fs.mkdir(path.join(directory, ".codeabz"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "codeabz.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "codeabz.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "codeabz.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "codeabz.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "codeabz.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".codeabz", "codeabz.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".codeabz", "codeabz.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".codeabz")),
              AbsolutePath.make(path.join(directory, ".codeabz")),
            ])
