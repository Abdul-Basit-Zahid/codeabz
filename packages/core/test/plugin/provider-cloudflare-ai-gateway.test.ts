...
import { CloudflareAIGatewayPlugin } from "@codeabz/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            ...
            metadata: { invoked_by: "test", project: "codeabz" },
            cacheTtl: 300,
            cacheKey: "cache-key",
            skipCache: true,
            collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("codeabz/"),
            },
          },
        })
      }),
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            name: "cloudflare-ai-gateway",
            headers: {
              "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "codeabz" }),
            },
          },
        })

        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "codeabz" },
        })
      }),
    ),
  )
