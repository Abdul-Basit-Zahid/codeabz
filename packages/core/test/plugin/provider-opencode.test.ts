...
import { codeabzPlugin } from "@codeabz/core/plugin/provider/codeabz"
...
const addPlugin = Effect.fn(function* () {
  ...
  yield* codeabzPlugin.effect(host).pipe(
...
describe("codeabzPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("codeabz")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "codeabz Console account",
        },
  ...
  it.live("loads providers and models from the connected codeabz server", () =>
      ...
      ({ authorization, release, server }) =>
        Effect.gen(function* () {
          ...
          yield* credentials.create({
            integrationID: Integration.ID.make("codeabz"),
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("codeabz"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "codeabz",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ CODEABZ_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.codeabz),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.codeabz)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.codeabz, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ CODEABZ_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.codeabz)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.codeabz, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ CODEABZ_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.codeabz)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.codeabz, ModelV2.ID.make("output-only"))).enabled).toBe(
          true,
        )
      }),
    ),
  ...
  it.effect("uses CODEABZ_API_KEY as credentials", () =>
    withEnv({ CODEABZ_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.codeabz)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.codeabz, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ CODEABZ_API_KEY: undefined, CUSTOM_CODEABZ_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("codeabz"),
            method: { type: "env", names: ["CUSTOM_CODEABZ_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ CODEABZ_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.codeabz)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.codeabz, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-codeabz providers and models", () =>
    withEnv({ CODEABZ_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the codeabz small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.codeabz
