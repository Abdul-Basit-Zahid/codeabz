...
import { ModelsDevPlugin } from "@codeabz/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CODEABZ_MODELS_PATH,
          disabled: Flag.CODEABZ_DISABLE_MODELS_FETCH,
        }
        Flag.CODEABZ_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CODEABZ_MODELS_PATH = previous.path
          Flag.CODEABZ_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
