...
const codeabzClient = process.env.CODEABZ_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (codeabzClient === undefined) delete process.env.CODEABZ_CLIENT
  else process.env.CODEABZ_CLIENT = codeabzClient
})
...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["codeabz.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.CODEABZ_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "codeabz.client=web,service.instance.id=override,service.namespace=anomalyco"
    ...
    expect(resource().attributes["codeabz.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "codeabz-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "codeabz.log")
