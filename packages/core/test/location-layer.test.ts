...
import { define } from "@codeabz/plugin/v2/effect"
...
import { LocationServiceMap } from "@codeabz/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "codeabz.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "codeabz.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
