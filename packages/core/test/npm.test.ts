...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@codeabz/acme")).toBe("@codeabz/acme")
    expect(Npm.sanitize("@codeabz/acme@1.0.0")).toBe("@codeabz/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/codeabz/acme.git"
    const expected = win ? "acme@git+https_//github.com/codeabz/acme.git" : spec
