...
import { PromptInput } from "@codeabz/schema/prompt-input"
import { Session } from "@codeabz/schema/session"
import { Project } from "@codeabz/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@codeabz/schema/schema"
...
import { SessionEvent } from "@codeabz/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this codeabz process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this codeabz process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
