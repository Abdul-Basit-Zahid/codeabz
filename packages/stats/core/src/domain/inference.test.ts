...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve codeabz route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "codeabz")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "codeabz")).toBe("openai")
    expect(statProvider("big-pickle", "", "codeabz")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "codeabz"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep codeabz as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "codeabz"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "codeabz"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep codeabz or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "codeabz"), country: "US" })).toMatchObject([
