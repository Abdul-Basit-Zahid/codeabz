...
import type { DesktopTheme } from "@codeabz/ui/theme/types"
...
function windowDataFile(id: string) {
  return `codeabz.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "codeabz window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("codeabz is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
