...
import { expectcodeabzVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolvecodeabz?: typeof resolveWslcodeabz
...
export function createWslServersController(
  ...
  const setcodeabzCheck = (distro: string, check: WslcodeabzCheck) => {
    setState({
      codeabzChecks: {
        ...state.codeabzChecks,
        [distro]: check,
  ...
  const checkcodeabz = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolvecodeabz ?? resolveWslcodeabz)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return codeabzCheck(distro, resolved, version, appVersion)
  }

  const refreshcodeabzCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setcodeabzCheck(distro, await checkcodeabz(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const codeabzChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.codeabzChecks[distro])
        .map(async (distro) => [distro, await checkcodeabz(distro, opts)] as const),
    )
    if (codeabzChecks.length) {
      setState({ codeabzChecks: { ...state.codeabzChecks, ...Object.fromEntries(codeabzChecks) } })
    }
  }
  ...
  const refreshcodeabzCheckBackground = (id: string, distro: string) => {
    void checkcodeabz(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setcodeabzCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl codeabz check failed", { id, distro, message })
      })
  }

  const refreshcodeabzChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkcodeabz(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setcodeabzCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl codeabz check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshcodeabzCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installcodeabz(name: string) {
      await runJob({ kind: "install-codeabz", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslcodeabz(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "codeabz installation failed")
        }
        await refreshcodeabzCheck(name, { signal: abort.signal })
        expectcodeabzVersion(state.codeabzChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.codeabzChecks, distro) : {}),
      })
    },
...
function codeabzCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "codeabz is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "codeabz is installed but could not run",
    }
  }
  return {
    distro,
