...
let releasecodeabzResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectcodeabzVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectcodeabzVersion("1.14.35", "1.16.2")).toThrow(
    "codeabz update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating codeabz", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.codeabz/bin/codeabz",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, codeabzChecks: {} })
})
...
test("ignores stale background codeabz checks after removing a WSL server", async () => {
  persistedServers = []
  releasecodeabzResolve = undefined
  ...
  await waitFor(() => !!releasecodeabzResolve)
  await controller.removeServer("wsl:Debian")
  releasecodeabzResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().codeabzChecks).toEqual({})
})

test("ignores stale startup codeabz checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking codeabz", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const codeabz: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolvecodeabz: async (distro) => {
      codeabz.push(distro)
      return "/home/me/.codeabz/bin/codeabz"
    },
  })
  ...
  expect(codeabz).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(codeabz).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().codeabzChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check codeabz in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const codeabz: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(codeabz).toEqual(["Debian"])
  expect(Object.keys(controller.getState().codeabzChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolvecodeabz: async () => {
      await new Promise<void>((resolve) => {
        releasecodeabzResolve = resolve
      })
      return "/home/me/.codeabz/bin/codeabz"
    },
  }
