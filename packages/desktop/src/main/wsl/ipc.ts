...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-codeabz", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installcodeabz(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-codeabz", unavailable)
