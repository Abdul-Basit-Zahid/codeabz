...
import { type WslCommandLine, resolveWslcodeabz, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const codeabz = await resolveWslcodeabz(distro)
  if (!codeabz) throw new Error(`codeabz is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "codeabz"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export CODEABZ_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CODEABZ_CLIENT=desktop",
    `export CODEABZ_SERVER_USERNAME=${shellEscape(username)}`,
    `export CODEABZ_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(codeabz)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
