...
export async function installWslcodeabz(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://github.com/Abdul-Basit-Zahid/codeabz/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslcodeabz(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.codeabz/bin/codeabz" ]; then printf "%s\\n" "$HOME/.codeabz/bin/codeabz"; fi',
        distro,
        opts,
      )
    ).stdout,
