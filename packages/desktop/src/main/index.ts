...
const TEST_ONBOARDING = process.env.CODEABZ_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CODEABZ_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.codeabz.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `codeabz-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "codeabz Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codeabz://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("codeabz")
  registerRendererProtocol()
  setDockIcon()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.CODEABZ_PORT
    if (fromEnv) {
