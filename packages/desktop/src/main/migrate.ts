...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codeabz.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codeabz.settings.dat` is special: it maps to the `codeabz.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codeabz.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codeabz.settings.dat → the electron settings store ("codeabz.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codeabz.global.dat").
  const storeName = filename === "codeabz.settings.dat" ? "codeabz.settings" : filename
