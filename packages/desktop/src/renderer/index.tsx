...
import type { UpdaterState } from "@codeabz/app/updater"
...
import "./styles.css"
import { Splash } from "@codeabz/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CODEABZ_CHANNEL === "prod" &&
...
const deepLinkEvent = "codeabz:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CODEABZ__ ??= {}
  const pending = window.__CODEABZ__.deepLinks ?? []
  window.__CODEABZ__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `codeabz.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://github.com/Abdul-Basit-Zahid/codeabz/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("codeabz.global.dat").getItem("language")
