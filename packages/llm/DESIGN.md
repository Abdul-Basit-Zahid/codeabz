...
> current private `@codeabz/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@codeabz/ai`
...
- Secondary audience: codeabz and other durable agent runtimes
...
import { LLM } from "@codeabz/ai"
import { OpenAI } from "@codeabz/ai/providers/openai"
...
```ts
import { Message } from "@codeabz/ai"
...
import { LLM, Tool } from "@codeabz/ai"
...
codeabz and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@codeabz/ai/promise"
import { OpenAI } from "@codeabz/ai/providers/openai"
...
```ts
import { LLMSchema } from "@codeabz/ai/schema"
...
```ts
import { Provider, Protocol } from "@codeabz/ai/provider"
...
@codeabz/ai/providers/openai
@codeabz/ai/providers/anthropic
@codeabz/ai/providers/google
...
```
...
codeabz should migrate to `generateTurn` / `streamTurn`, preserving its durable
