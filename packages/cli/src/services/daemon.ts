...
import { createcodeabzClient } from "@codeabz/sdk/v2/client"
import { ServerAuth } from "@codeabz/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createcodeabzClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@codeabz/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createcodeabzClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createcodeabzClient({ baseUrl: connection.url, headers: connection.headers })
    })
