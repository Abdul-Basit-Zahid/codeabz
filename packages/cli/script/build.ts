...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      CODEABZ_CLI_NAME: `'${binary}'`,
      CODEABZ_MODELS_DEV: modelsData,
      CODEABZ_CHANNEL: `'${Script.channel}'`,
      CODEABZ_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/Abdul-Basit-Zahid/codeabz.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
