...
import { and, Database, eq, isNull, lt, or, sql } from "@codeabz/console-core/drizzle/index.js"
import { KeyTable } from "@codeabz/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@codeabz/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@codeabz/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@codeabz/console-core/util/date.js"
import { Identifier } from "@codeabz/console-core/identifier.js"
import { Billing } from "@codeabz/console-core/billing.js"
import { Actor } from "@codeabz/console-core/actor.js"
import { WorkspaceTable } from "@codeabz/console-core/schema/workspace.sql.js"
import { ZenData } from "@codeabz/console-core/model.js"
import { Subscription } from "@codeabz/console-core/subscription.js"
import { BlackData } from "@codeabz/console-core/black.js"
import { UserTable } from "@codeabz/console-core/schema/user.sql.js"
import { ModelTable } from "@codeabz/console-core/schema/model.sql.js"
import { ProviderTable } from "@codeabz/console-core/schema/provider.sql.js"
...
import { LiteData } from "@codeabz/console-core/lite.js"
import { Resource } from "@codeabz/console-resource"
...
import { Workspace } from "@codeabz/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-codeabz-session") ?? ""
    const requestId = input.request.headers.get("x-codeabz-request") ?? ""
    const ocClient = input.request.headers.get("x-codeabz-client") ?? ""
    const projectId = input.request.headers.get("x-codeabz-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://github.com/Abdul-Basit-Zahid/codeabz/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-codeabz-request")
            headers.delete("x-codeabz-session")
            headers.delete("x-codeabz-project")
            headers.delete("x-codeabz-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-codeabz-endpoint-id")
        const resEndpointModelId = res.headers.get("x-codeabz-upstream-model-id")
    ...
    if (modelData.trialEnded)
      throw new ModelError(
        `${t("zen.api.error.trialEnded", {
          ...
          link: "https://github.com/Abdul-Basit-Zahid/codeabz/go",
        })}`,
      )
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://github.com/Abdul-Basit-Zahid/codeabz/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://github.com/Abdul-Basit-Zahid/codeabz/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://github.com/Abdul-Basit-Zahid/codeabz/workspace/${authInfo.workspaceID}/members`
