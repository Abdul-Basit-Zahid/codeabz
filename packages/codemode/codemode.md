...
This is the living design and status document for `@codeabz/codemode` and its existing V2 codeabz adapter.
...
`packages/codeabz/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
