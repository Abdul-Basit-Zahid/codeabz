...
import { registercodeabzSpinner } from "./component/register-spinner"
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.CODEABZ_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.CODEABZ_ROUTE ? JSON.parse(process.env.CODEABZ_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.CODEABZ_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CODEABZ_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CODEABZ_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("CodeABZ")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.CODEABZ_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://github.com/Abdul-Basit-Zahid/codeabz/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to codeabz v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.CODEABZ_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.CODEABZ_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
