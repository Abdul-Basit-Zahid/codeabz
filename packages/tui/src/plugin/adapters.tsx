import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@codeabz/plugin/tui"
...
import type { useCodeabzKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCodeabzKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCodeabzModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCodeabzModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
