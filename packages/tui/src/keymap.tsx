...
export const CODEABZ_BASE_MODE = "base"
...
export function createcodeabzModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CODEABZ_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CODEABZ_MODE_KEY, stack.at(-1)?.mode ?? CODEABZ_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CODEABZ_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CODEABZ_MODE_KEY, undefined)
