...
import { getcodeabzModeStack, CODEABZ_BASE_MODE, codeabzKeymapProvider, registercodeabzKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registercodeabzKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when codeabz mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getcodeabzModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getcodeabzModeStack(keymap).push("autocomplete")
