...
- Author package entrypoint is `@codeabz/plugin/tui`.
...
```json
{
  "$schema": "https://github.com/Abdul-Basit-Zahid/codeabz/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/codeabz-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@codeabz/plugin/tui`.
- `@codeabz/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@codeabz/plugin/tui"
...
- The value is a semver range checked against the running codeabz version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codeabz` is absent, no check is performed (backward compatible).
...
- `codeabz plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codeabz plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codeabz` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codeabz`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codeabz`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codeabz` dir or `codeabz_CONFIG_DIR`, codeabz installs `@codeabz/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codeabz/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus codeabz's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
codeabz registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@codeabz/plugin/tui`.
...
- `message` is required; `title` defaults to `"codeabz"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `codeabz.default`.
...
- Local plugins persist installed themes under the local `.codeabz/themes` area near the plugin config source.
...
- `--pure` / `codeabz_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codeabz/plugins/tui-smoke.tsx`
- Local vim plugin: `.codeabz/plugins/tui-vim.tsx`
- Local smoke config: `.codeabz/tui.json`
- Local smoke theme: `.codeabz/plugins/smoke-theme.json`
