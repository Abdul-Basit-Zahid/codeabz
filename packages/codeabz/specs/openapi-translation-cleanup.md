...
Trim `packages/codeabz/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/codeabz/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codeabz`.
- `[x]` `bun typecheck` from `packages/codeabz`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/codeabz/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codeabz`.
- `[x]` `bun dev generate > /tmp/codeabz-openapi.json` from `packages/codeabz`.
...
- Edit `packages/codeabz/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/codeabz-openapi.json` from `packages/codeabz`.
...
- Check source schemas in files like `packages/codeabz/src/session/schema.ts`, `packages/codeabz/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/codeabz/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/codeabz/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
