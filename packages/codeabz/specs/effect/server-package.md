...
Practical reference for a future `packages/server` split after the codeabz
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/codeabz`.
...
Until enough shared service code lives outside `packages/codeabz`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/codeabz`

It should not import `packages/codeabz` services while `packages/codeabz`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on codeabz-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/codeabz` implementation details.
