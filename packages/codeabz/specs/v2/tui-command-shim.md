...
Delete `packages/codeabz/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/codeabz/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/codeabz/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/codeabz`
- TUI plugin loader tests in `packages/codeabz` if runtime plugin API wiring changed
