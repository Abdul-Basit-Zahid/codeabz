...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-codeabz-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-codeabz-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://github.com/Abdul-Basit-Zahid/codeabz/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codeabz-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codeabz-workspace")
