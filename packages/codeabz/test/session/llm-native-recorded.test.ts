...
import { SessionV1 } from "@codeabz/core/v1/session"
import { ModelsDev } from "@codeabz/core/models-dev"
import { HttpRecorder } from "@codeabz/http-recorder"
import { HttpRecorderInternal } from "@codeabz/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@codeabz/llm"
import { RequestExecutor } from "@codeabz/llm/route"
...
const zenURL = (connection: string) => `https://console.github.com/Abdul-Basit-Zahid/codeabz/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CODEABZ_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["codeabz", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODEABZ_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CODEABZ_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["codeabz", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "codeabz-proxy",
    name: "codeabz proxy",
    providerID: ProviderV2.ID.codeabz,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["codeabz", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CODEABZ_RECORD_CONSOLE_TOKEN && process.env.CODEABZ_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.codeabz,
        name: "codeabz Zen",
        env: ["CODEABZ_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CODEABZ_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CODEABZ_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CODEABZ_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["codeabz", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODEABZ_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CODEABZ_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CODEABZ_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CODEABZ_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the codeabz LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "codeabz.json"),
      JSON.stringify({ $schema: "https://github.com/Abdul-Basit-Zahid/codeabz/config.json", ...scenario.config(model) }),
    ),
  )
