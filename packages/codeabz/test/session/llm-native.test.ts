...
import { LLMEvent, ToolFailure } from "@codeabz/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@codeabz/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("codeabz"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("codeabz") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("codeabz") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, codeabz, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored codeabz auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("codeabz") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("codeabz"),
