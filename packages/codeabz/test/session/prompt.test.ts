...
import { SessionMessageTable } from "@codeabz/core/session/sql"
...
import { Shell } from "@codeabz/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@codeabz/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "codeabz.json"),
    JSON.stringify({ $schema: "https://github.com/Abdul-Basit-Zahid/codeabz/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/codeabz/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("codeabz"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
