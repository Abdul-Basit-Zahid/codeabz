...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/codeabz.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/codeabz.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/codeabz.jsonc", dir: ".codeabz", suggestion: "codeabz" },
        expected:
          'Directory ".codeabz" in /tmp/codeabz.jsonc is not valid. Rename the directory to "codeabz" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/codeabz.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/codeabz.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.github.com/Abdul-Basit-Zahid/codeabz/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.github.com/Abdul-Basit-Zahid/codeabz/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `codeabz models` to list available models",
      "Or check your config (codeabz.json) provider/model names",
    ].join("\n")
