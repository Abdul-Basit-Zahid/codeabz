...
import { codeabzClient, type GlobalEvent } from "@codeabz/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<codeabzClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<codeabzClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<codeabzClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<codeabzClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<codeabzClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://codeabz.test"),
...
function sdk(
  input: {
    ...
    subscribe?: codeabzClient["event"]["subscribe"]
    globalEvent?: codeabzClient["global"]["event"]
    promptAsync?: codeabzClient["session"]["promptAsync"]
    status?: codeabzClient["session"]["status"]
    messages?: codeabzClient["session"]["messages"]
    children?: codeabzClient["session"]["children"]
    permissions?: codeabzClient["permission"]["list"]
    questions?: codeabzClient["question"]["list"]
  } = {},
) {
  const client = new codeabzClient()

  const subscribe: codeabzClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: codeabzClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: codeabzClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: codeabzClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: codeabzClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: codeabzClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: codeabzClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: codeabzClient["question"]["list"] = input.questions ?? (() => ok([]))
