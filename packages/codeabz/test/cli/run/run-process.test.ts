// Subprocess integration tests for `codeabz run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `codeabz.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CODEABZ_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("codeabz run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("say hi")
        codeabz.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* codeabz.run("think", { extraArgs: ["--thinking"] })
        codeabz.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* codeabz.run("think again")
        codeabz.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("say hi", { format: "json" })
        codeabz.expectExit(result, 0)

        const events = codeabz.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const result = yield* codeabz.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codeabz.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* codeabz.run("request permission", { permission: { bash: "ask" } })
        codeabz.expectExit(denied, 0)
        ...
        const allowed = yield* codeabz.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        codeabz.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* codeabz.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        codeabz.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, codeabz }) =>
      Effect.gen(function* () {
        ...
        const server = yield* codeabz.serve()

        const result = yield* codeabz.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, codeabz }) =>
      Effect.gen(function* () {
        const result = yield* codeabz.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, codeabz }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* codeabz.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
