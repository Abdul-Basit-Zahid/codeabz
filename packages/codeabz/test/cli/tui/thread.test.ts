...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ codeabz }) =>
    Effect.gen(function* () {
      const result = yield* codeabz.spawn(["--replay-limit", "10"])

      codeabz.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ codeabz }) =>
    Effect.gen(function* () {
      const result = yield* codeabz.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ codeabz }) =>
    Effect.gen(function* () {
      const result = yield* codeabz.spawn(["--mini", "--port", "4096"])
