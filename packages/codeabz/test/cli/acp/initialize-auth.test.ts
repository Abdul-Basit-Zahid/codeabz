...
describe("codeabz acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ codeabz }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ codeabz }))
        ...
        expect(initialized.agentInfo?.name).toBe("codeabz")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ codeabz })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("codeabz-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "codeabz-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CODEABZ_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("codeabz-login")
