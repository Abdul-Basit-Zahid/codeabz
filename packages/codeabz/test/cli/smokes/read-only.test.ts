...
describe("codeabz read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CODEABZ_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["mcp", "list"])
        codeabz.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["providers", "list"])
        codeabz.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["models"])
        codeabz.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["agent", "list"])
        codeabz.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CODEABZ_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["session", "list"])
        codeabz.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["stats"])
        codeabz.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CODEABZ_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ codeabz }) =>
      Effect.gen(function* () {
        const r = yield* codeabz.spawn(["db", "path"])
        codeabz.expectExit(r, 0, "db path")
