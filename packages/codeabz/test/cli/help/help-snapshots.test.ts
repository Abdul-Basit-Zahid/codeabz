...
// terminal sizes. The default codeabz tui command is excluded —
// `codeabz --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `codeabz --help` prints today;
...
// top-level help on `--help` and exits 1; not a real codeabz command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("codeabz CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ codeabz }) =>
      Effect.gen(function* () {
        const topLevel = yield* codeabz.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* codeabz.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`codeabz ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`codeabz ${argv.join(" ")} --help`)
        }
