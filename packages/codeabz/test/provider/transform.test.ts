    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".codeabz/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const codeabzModel = {
      ...
      api: {
        id: "codeabz-test",
        url: "https://api.github.com/Abdul-Basit-Zahid/codeabz",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, codeabzModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.codeabz?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.codeabz?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              codeabz: { itemId: "msg_CODEABZ_part" },
    ...
    expect(result[0].providerOptions?.codeabz?.itemId).toBe("msg_codeabz")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.codeabz?.itemId).toBe("msg_CODEABZ_part")
