...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CODEABZ_TEST ?? "")'), {
      env: {
        CODEABZ_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CODEABZ_TEST_SHELL"], {
      shell: true,
      env: {
        CODEABZ_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CODEABZ_TEST_SHELL=ok")
  })
