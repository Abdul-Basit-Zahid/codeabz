...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codeabz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codeabz.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codeabz.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codeabz.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codeabz.json", "codeabz.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "codeabz.json"),
        path.join(project, "codeabz.jsonc"),
      ])
    })
  })
