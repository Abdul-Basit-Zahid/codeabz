...
import { EventSequenceTable } from "@codeabz/core/event/sql"
...
import { WorkspaceV2 } from "@codeabz/core/workspace"
import { WorkspaceTable } from "@codeabz/core/control-plane/workspace.sql"
...
const originalEnv = {
  CODEABZ_AUTH_CONTENT: process.env.CODEABZ_AUTH_CONTENT,
  CODEABZ_EXPERIMENTAL_WORKSPACES: process.env.CODEABZ_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CODEABZ_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@codeabz.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CODEABZ_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=codeabz-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CODEABZ_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CODEABZ_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CODEABZ_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=codeabz-test")
