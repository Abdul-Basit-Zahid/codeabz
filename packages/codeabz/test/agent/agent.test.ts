...
it.instance("plan agent denies edits except .codeabz/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".codeabz/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".codeabz", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CODEABZ_TEST_HOME
      process.env.CODEABZ_TEST_HOME = test.directory
