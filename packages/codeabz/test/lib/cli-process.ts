// Subprocess test harness for the codeabz CLI. Spawns the real binary against
...
//
// Configuration flows through codeabz's built-in test affordances:
//   - CODEABZ_CONFIG_CONTENT      : provider config inline, no files to find
//   - CODEABZ_TEST_HOME           : pins os.homedir() → tmpdir
//   - CODEABZ_DISABLE_PROJECT_CONFIG : skip walking up for codeabz.json
//   - CODEABZ_PURE                : skip external plugin discovery + install
//   - CODEABZ_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `codeabz.run` is fully wired. The shape supports adding more
// builders (`codeabz.serve(opts)`, `codeabz.acp(opts)`, `codeabz.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of codeabzCli.
...
import { AppProcess } from "@codeabz/core/process"
...
const codeabzRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(codeabzRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CODEABZ_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CODEABZ_CONFIG_CONTENT: configJson,
    CODEABZ_DISABLE_PROJECT_CONFIG: "1",
    CODEABZ_PURE: "1",
    CODEABZ_DISABLE_AUTOUPDATE: "1",
    CODEABZ_DISABLE_AUTOCOMPACT: "1",
    CODEABZ_DISABLE_MODELS_FETCH: "1",
    CODEABZ_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `codeabz run`. New flags should
...
// `codeabz serve` is a long-lived process — it never exits on its own.
...
// `codeabz acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type codeabzCli = {
  ...
  // Spawn `codeabz serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `codeabz acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("codeabz.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CODEABZ_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("codeabz.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("codeabz.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "codeabz server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `codeabz serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("codeabz.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const codeabz: codeabzCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, codeabz })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `codeabz.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "codeabz") {
...
// every call site. Use this for any test that needs the codeabz CLI fixture.
//
...
// resources (e.g. `codeabz.serve`) without an extra `Effect.scoped` wrapper —
