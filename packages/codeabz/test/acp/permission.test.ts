...
import type { Event, codeabzClient } from "@codeabz/sdk/v2"
...
type PermissionReplyParams = Parameters<codeabzClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "codeabz-acp-permission-"))
