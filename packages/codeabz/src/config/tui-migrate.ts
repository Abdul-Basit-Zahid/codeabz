...
const TUI_SCHEMA_URL = "https://github.com/Abdul-Basit-Zahid/codeabz/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from codeabz.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const codeabz = await codeabzFiles(input)
  for (const file of codeabz) {
...
async function codeabzFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "codeabz"),
    ...(await Filesystem.findUp(["codeabz.json", "codeabz.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "codeabz"))
  }
  if (Flag.CODEABZ_CONFIG) files.push(Flag.CODEABZ_CONFIG)
