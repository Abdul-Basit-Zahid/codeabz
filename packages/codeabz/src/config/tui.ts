...
import { isRecord } from "@codeabz/tui/util/record"
...
import { TuiKeybind } from "@codeabz/tui/config/keybind"
...
import { TuiConfig } from "@codeabz/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@codeabz/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old codeabz.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.codeabz`
  // folders between cwd and home, and CODEABZ_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CODEABZ_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CODEABZ_TUI_CONFIG override, if set.
  if (Flag.CODEABZ_TUI_CONFIG) {
    const configFile = Flag.CODEABZ_TUI_CONFIG
  ...
  // 4. `.codeabz` directories (and CODEABZ_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".codeabz") || dir === Flag.CODEABZ_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".codeabz") && dir !== Flag.CODEABZ_CONFIG_DIR) continue
