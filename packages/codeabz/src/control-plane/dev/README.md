This is a plugin to simulate a remote environment locally. Add this to `.codeabz/codeabz.jsonc`:

```json
  "plugin": ["../packages/codeabz/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate codeabz server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/codeabz/script/run-workspace-server
```

With the plugin install, you can now run codeabz and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
