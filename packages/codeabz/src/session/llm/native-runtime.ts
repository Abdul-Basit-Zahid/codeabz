...
import type { LLMClientShape } from "@codeabz/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("codeabz"))
    return { type: "unsupported", reason: "provider is not openai, codeabz, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @codeabz/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains codeabz-owned. The native runtime only adapts
      // the @codeabz/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
