...
import { codeabzHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for codeabz serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(codeabzHttpApi)
...
export const PublicApi = codeabzHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "codeabz",
    version: "1.0.0",
    description: "codeabz api",
