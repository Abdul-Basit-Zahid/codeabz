...
const AGENT_USERNAME = "codeabz-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/codeabz.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent - https://github.com/Abdul-Basit-Zahid/codeabz/docs/github/#usage-examples",
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/codeabz-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.github.com/Abdul-Basit-Zahid/codeabz/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  codeabz:
    if: |
      ...
      startsWith(github.event.comment.body, '/codeabz')
    ...
    steps:
      ...
      - name: Run codeabz
        uses: Abdul-Basit-Zahid/codeabz/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.github.com/Abdul-Basit-Zahid/codeabz" : "https://github.com/Abdul-Basit-Zahid/codeabz"
    ...
    try {
      ...
      console.log("codeabz session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.github.com/Abdul-Basit-Zahid/codeabz"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/codeabz,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to codeabz...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("codeabz-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `codeabz/${type}-${hex}-${timestamp}`
      }
      return `codeabz/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/codeabz-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[codeabz session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the codeabz infrastructure after your response",
