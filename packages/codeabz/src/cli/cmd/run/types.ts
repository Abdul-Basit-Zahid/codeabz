// Shared type vocabulary for the direct interactive mode (`codeabz --mini`).
//
...
import type { codeabzClient, PermissionRequest, QuestionRequest, ToolPart } from "@codeabz/sdk/v2"
import type { TuiConfig } from "@codeabz/tui/config"
...
type PromptModel = Parameters<codeabzClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<codeabzClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<codeabzClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<codeabzClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<codeabzClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<codeabzClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<codeabzClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<codeabzClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<codeabzClient["question"]["reject"]>[0]
