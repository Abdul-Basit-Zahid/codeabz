...
import type { AssistantMessage, Message, codeabzClient, SessionMessageResponse } from "@codeabz/sdk/v2"
...
export const AuthMethodID = "codeabz-login"
...
export class Service extends Context.Service<Service, Interface>()("@codeabz/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `codeabz auth login` in the terminal",
      name: "Login with codeabz",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: codeabzClient) {
...
function makeUsageService(sdk: codeabzClient) {
...
async function loadDirectorySnapshot(sdk: codeabzClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, codeabz provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const codeabzProvider = providers[ProviderV2.ID.make("codeabz")]
  const codeabzModel = codeabzProvider ? Provider.sort(Object.values(codeabzProvider.models))[0] : undefined
  if (codeabzProvider && codeabzModel) return { providerID: codeabzProvider.id, modelID: codeabzModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "codeabz service failure", service })
}
