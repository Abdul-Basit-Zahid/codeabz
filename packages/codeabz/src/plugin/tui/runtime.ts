...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@codeabz/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@codeabz/core/effect/app-node-builder"
import { errorData, errorMessage } from "@codeabz/tui/util/error"
...
import { hasTheme, upsertTheme } from "@codeabz/tui/context/theme"
import { Global } from "@codeabz/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@codeabz/core/util/flock"
import { Flag } from "@codeabz/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@codeabz/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@codeabz/core/v1/config/plugin"
import { createCommandShim } from "@codeabz/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@codeabz/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".codeabz"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".codeabz", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codeabz", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CODEABZ_PURE ? [] : pluginOrigins
    if (Flag.CODEABZ_PURE && pluginOrigins.length) {
    }
