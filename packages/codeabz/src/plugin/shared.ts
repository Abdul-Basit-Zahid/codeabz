...
export const DEPRECATED_PLUGIN_PACKAGES = ["codeabz-openai-codex-auth", "codeabz-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, codeabzVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codeabzVersion) || semver.major(codeabzVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.codeabz
  if (typeof range !== "string") return
  if (!semver.satisfies(codeabzVersion, range)) {
    throw new Error(`Plugin requires codeabz ${range} but running ${codeabzVersion}`)
  }
}
