...
Move the canonical codeabz terminal application from
`packages/codeabz/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/codeabz`,
`packages/cli`, or `@codeabz/core`.

The SDK is the TUI's codeabz boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/codeabz`.
- Do not replace private `packages/codeabz` imports with `@codeabz/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@codeabz/tui`.
...
- codeabz domain data displayed by the TUI
...
```text
packages/codeabz/src/cli/cmd/tui
```

Its private dependency on `packages/codeabz` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/codeabz/src/*`.
...
Status: Completed. The private `@codeabz/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@codeabz/tui`.
...
- The package has no dependency on `codeabz`, `@codeabz/cli`, or
  `@codeabz/core`.
...
primitives now live in `@codeabz/tui`. The legacy host consumes explicit
...
and build inputs through `@codeabz/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads codeabz globals or process state;
...
Replace process-global codeabz state with resolved TUI inputs.
...
`@codeabz/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/codeabz` initially.
...
canonical in `@codeabz/tui`. Configured references resolve through the new
...
adapters rather than codeabz domain implementations.

Make the SDK the only codeabz domain boundary used by the TUI.
...
- Domain-facing TUI code imports codeabz data and operations only from
  `@codeabz/sdk`.
...
`@codeabz/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/codeabz`.
...
- The package has no imports from `packages/codeabz`, `packages/cli`, or
  `@codeabz/core`.
...
public `@codeabz/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/codeabz`.
- Keep the legacy embedded worker and server startup in `packages/codeabz`.
- Change those adapters to load config, create transport inputs, and call the
  public `@codeabz/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/codeabz`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@codeabz/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@codeabz/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for codeabz domain state.
...
cd packages/codeabz && bun typecheck
...
rg '@codeabz/core|packages/codeabz|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/codeabz/src packages/codeabz/test
```
...
- Build the current-platform `packages/codeabz` binary.
