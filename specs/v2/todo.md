...
The codeabz server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@codeabz/llm` tool loop after replacing its
...
- plugins get global "codeabz" instance like in that post i showed
- codeabz instance has stuff like `codeabz.session.prompt()` or
  `codeabz.tool.register({...})`
...
- decide whether to preserve deprecated `@codeabz/llm` orchestration exports
