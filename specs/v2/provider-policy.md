...
Both `action` and `resource` use codeabz's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.codeabz` files is intentionally deferred until `.codeabz` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether codeabz permits an operation through its managed execution paths.
