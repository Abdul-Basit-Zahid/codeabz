# Remove `packages/codeabz/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/codeabz/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the codeabz legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/codeabz/src/storage/db.ts` are concentrated in 22 source files:

- `packages/codeabz/src/account/repo.ts`
- `packages/codeabz/src/cli/cmd/db.ts`
- `packages/codeabz/src/cli/cmd/import.ts`
- `packages/codeabz/src/cli/cmd/stats.ts`
- `packages/codeabz/src/control-plane/workspace.ts`
- `packages/codeabz/src/index.ts`
- `packages/codeabz/src/node.ts`
- `packages/codeabz/src/permission/index.ts`
- `packages/codeabz/src/project/project.ts`
- `packages/codeabz/src/server/projectors.ts`
- `packages/codeabz/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/codeabz/src/server/shared/fence.ts`
- `packages/codeabz/src/session/message-v2.ts`
- `packages/codeabz/src/session/projectors.ts`
- `packages/codeabz/src/session/prompt.ts`
- `packages/codeabz/src/session/session.ts`
- `packages/codeabz/src/session/todo.ts`
- `packages/codeabz/src/share/share-next.ts`
- `packages/codeabz/src/storage/db.ts`
- `packages/codeabz/src/sync/index.ts`
- `packages/codeabz/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy codeabz database wrapper; `packages/codeabz/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the codeabz wrapper.
...
Status: Completed. `SyncEvent` and the codeabz projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy codeabz database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy codeabz database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy codeabz database wrapper.
...
All migration groups are complete or superseded. `packages/codeabz/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No codeabz data-migration group remains.

The previous codeabz `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate codeabz data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/codeabz`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/codeabz/src`
- `bun typecheck` from `packages/codeabz`
- Relevant package tests from `packages/codeabz`, not the repo root
